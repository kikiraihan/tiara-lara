<div class="w-64 bg-white border-r border-gray-200 overflow-y-auto">
    ...
    <nav class="p-4 space-y-1">
        @foreach ($navItems as $item)
            ...
            <a href="{{ $item['href'] }}"
                ...
                <i data-lucide="{{ $item['icon'] }}" class="h-5 w-5 mr-3 text-gray-500 {{ $isActive ? 'text-blue-600' : '' }}"></i>
                <span>{{ $item['text'] }}</span>
            </a>
        @endforeach
    </nav>
    ...
    <div class="p-4 border-t border-gray-200 mt-auto">
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button href="#"
                class="mt-4 w-full flex items-center justify-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-raisaDongker1 hover:bg-raisaDongker2">
