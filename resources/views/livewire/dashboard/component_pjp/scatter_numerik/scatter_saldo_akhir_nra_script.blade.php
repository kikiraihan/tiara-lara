...
<script>
    Livewire.on('reloadScatterSaldoAkhir', (fromServer) => {
        const data = fromServer[0];
        const elements = getElementSaldoAkhir();
        setElementSaldoAkhir(elements);
        createChartSaldoAkhir(elements.containerElement, data.data, data.data_regression);
    });

    function getElementSaldoAkhir() {
        return {
            containerElement: document.getElementById('container-scatter_saldo_akhir_nra'),
        };
    }

    function setElementSaldoAkhir(elements) {
    ...
    function createChartSaldoAkhir(element, data, data_regression) {
        const NRA_PRED = Object.values(data.NRA_PRED);
        const saldo_akhir = Object.values(data.saldo_akhir);
        ...
        const layout = {
            title: {
                text: 'Hubungan antara saldo_akhir dan NRA_PRED',
                font: {
                    size: 18,
                    color: textColor
                }
